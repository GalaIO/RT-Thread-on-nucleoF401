 ...
 *@param  slave_addr  input the specific device address, like 0xD0 for MPU6050 with AD0 = 0.
 *@param  reg_addr  	 tell IIC which register you want to operate.
 *@param  length      the data length you want to send.
 *@param  data        the data array addr you store the info.
 *@return 0:ok none error; not 0:some error occured.
 * unsigned char i2c_write(unsigned char slave_addr, unsigned char reg_addr,
 *      unsigned char length, unsigned char const *data)
 *
 *
 *@param  slave_addr  input the specific device address, like 0xD0 for MPU6050 with AD0 = 0.
 *@param  reg_addr  	tell IIC which register you want to operate.
 *@param  length      the data length you want to read.
 *@param  data        the data array addr you want to store.
 *@return 0:ok none error; not 0:some error occured.
 * unsigned char i2c_read(unsigned char slave_addr, unsigned char reg_addr,
 *      unsigned char length, unsigned char *data)
 *
 *@param  num_ms  delay num_ms.
 ...
 * min(int a, int b)
 */
#if defined MOTION_DRIVER_TARGET_RTTHREAD_STM32F4
#include "bsp_user.h"
#include "bsp_iic1.h"

#define i2c_write   IIC1writeBytes
#define i2c_read    IIC1readBytes
#define delay_ms    delayMS
#define get_ms(a)      //Null
/*static inline int reg_int_cb(struct int_param_s *int_param)
{
		rt_kprintf("int reg_int_cb.....\r\n");
    return 0;
}*/
...
int mpu_reset_fifo(void)
{
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &data))
            return -1;
				/*@added GalaIO ,reduce the delay of fifo_reset*/
        //delay_ms(50);
...
u8 run_self_test(void)
{
	...
	if (result == 0x3) 
	{
		...
		*/
		...
		accel[1] *= accel_sens;
		accel[2] *= accel_sens;
//		dmp_set_accel_bias(accel);	//校准初始角度
		return 0;
	}else return 1;
}
//陀螺仪方向控制
...
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	**/
	if(sensors&INV_WXYZ_QUAT) 
	{
		...
		*yaw   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * RAD_TO_DEG;	//yaw
	}else return 2;
	return 0;
}
